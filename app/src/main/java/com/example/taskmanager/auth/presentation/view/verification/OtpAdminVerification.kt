 ...
 */
@Composable
fun OtpAdminVerificationScreen(
    onVerifyClick: (SignUpAdminUiEvent) -> Unit,
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = state.adminSignupRequest.username,
            ...
            OutlinedTextField(
                value = state.adminSignupRequest.otpEmailVerifyCode,
                onValueChange = {
                    if (it.length <= 6 && it.all { char -> char.isDigit() }) {
                        viewModel.onEvent(SignUpAdminUiEvent.OnOtpChange(it))
                    }
                },
            ...
            Button(
                onClick = {
                    onVerifyClick(
                        SignUpAdminUiEvent.SignUpAdmin(
                            state.adminSignupRequest
                        )
                    )
                },
                modifier = Modifier
                    .fillMaxWidth()
                    .height(50.dp),
                enabled = !state.isLoading && state.adminSignupRequest.otpEmailVerifyCode.length == 6,
                shape = MaterialTheme.shapes.medium
            ) {
                if (state.isLoading) {
                ...
                if (!canResend) {
                    ...
                    TextButton(
                        onClick = {
                            ...
                            timeLeft = 60
                            canResend = false
                            viewModel.onEvent(SignUpAdminUiEvent.OnVerifyEmail(state.adminSignupRequest.username.toVerificationRequestDto()))
                        }
                    ) {
                        Text("Resend Code")
                    }
